...
async function ensureGitActivated() {
...
export function activate(context: vscode.ExtensionContext) {
  class ChangedFilesProvider implements vscode.TreeDataProvider<string> {
    ...
    async getChildren(): Promise<string[]> {
      ...
      // "D  src/remove.ts"
      // "?? newfile.ts"
      const files = lines.map((line: string) => line.substring(3).trim());

      return files;
    }

    getTreeItem(filePath: string): vscode.TreeItem {
      const item = new vscode.TreeItem(filePath);
      item.command = {
  ...
  try {
    console.log("AI Review Extension Activated");
    ...
    vscode.commands.registerCommand("srv.jumpToComment", async (comment) => {
      ...
      try {
        ...
        const diff = "";
        applyReviewDecorations(editor, fileComments, diff);
      } catch (e) {
        ...
        return vscode.window.withProgress(
          ...
          async (progress) => {
            ...
            const targetFilePath =
              sourceFilePath ?? vscode.window.activeTextEditor?.document.fileName;

            if (!targetFilePath) {
              vscode.window.showErrorMessage("Không có file nào đang được mở.");
              return;
            }
            ...
            const diffArgs = trackedResult.error
              ? ["diff", "--no-ext-diff", "--no-color", "-U5", "--no-index", "NUL", relativePath]
              : ["diff", "--no-ext-diff", "--no-color", "-U5", "HEAD", "--", relativePath];
              ...
              messages: [
                {
                  role: "system",
                  content: `
You are an AI code reviewer. Return ONLY valid JSON.
...
`,
            ...
            console.log("AI RAW OUTPUT:", comment_string);
    ...
    vscode.commands.registerCommand("srv.applyFix", async (uri, comment) => {
      ...
      const findAnchorRange = (): {
        ...
        const lineMatches = (fileLine: string, beforeLine: string): boolean => {
        ...
        for (let i = 0; i <= fileLines.length - windowSize; i++) {
          ...
          if (matches) {
            ...
            const expandedEnd = expandToStatement(fullText, realStart, realEnd);
    ...
    vscode.commands.registerCommand("srv.deleteComment", async (uri, comment) => {
      if (typeof uri === "string") uri = vscode.Uri.parse(uri);
      ...
      try {
        ...
        for (const editor of targetEditors) {
          const fileComments = updated.filter((c: any) => c.file === targetFile);
          applyReviewDecorations(editor, fileComments, "");
        }
      } catch (err) {
        console.error("Decoration refresh failed after deleteComment:", err);
      }

      vscode.window.showInformationMessage("Đã xóa comment khỏi danh sách.");
    });
  } catch (err) {
